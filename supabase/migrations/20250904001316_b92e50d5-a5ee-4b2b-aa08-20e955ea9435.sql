...
-- 1. Enable RLS on spatial_ref_sys table with public read access
ALTER TABLE public.spatial_ref_sys ENABLE ROW LEVEL SECURITY;

-- Create policy to allow public read access to spatial reference systems (needed for PostGIS functionality)
CREATE POLICY "Allow public read access to spatial reference systems" 
ON public.spatial_ref_sys 
FOR SELECT 
USING (true);

-- 2. Fix search_path for remaining custom functions
