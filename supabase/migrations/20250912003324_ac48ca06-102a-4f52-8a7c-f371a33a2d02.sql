-- Melhorar as funções de verificação KYC e adicionar triggers de auditoria
...
-- Função para bloquear usuário por KYC suspeito
CREATE OR REPLACE FUNCTION block_user_kyc(
    target_user_id UUID,
    admin_user_id UUID,
    block_reason TEXT
)
...
BEGIN
    ...
    UPDATE profiles 
    SET 
        kyc_status = 'bloqueado',
        kyc_notes = block_reason,
        updated_at = NOW()
    ...
    INSERT INTO audit_logs (
        ...
        'profile',
        profile_record.id,
        jsonb_build_object('kyc_status', profile_record.kyc_status),
        jsonb_build_object('kyc_status', 'bloqueado'),
        jsonb_build_object(
    ...
    INSERT INTO admin_alerts (
        ...
        'user_blocked',
        'high',
        'Usuário bloqueado por KYC',
        'Usuário foi bloqueado devido a problemas na verificação de identidade',
        'profile',
        profile_record.id,
        jsonb_build_object(
